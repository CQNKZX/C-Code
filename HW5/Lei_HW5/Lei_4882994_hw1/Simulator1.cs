...
namespace Lei_4882994_hw1
{
    sealed class Simulator1//simulate price evolution and get results
    {
        ...
        public static double[,] priceevol(Option haha, double[,] hahaha, bool multi) //compute underlying price evolution and output state at maturity
        {
            //how many rows do we have
            int row = hahaha.GetLength(0);
            int col = hahaha.GetLength(1);
            //matrix that stores price change information
            //one more column because of initial price
            double[,] priceevol = new double[row, col + 1];
            if (multi == false)
            {
        ...
        static void fill_rows_price(object notreally)//Again, not really all objects fit in
        {
            ...
            double timestep = lala.Opt.T / step; //time increment
            double rate = lala.Opt.R;
            double vol = lala.Opt.Sig;

            //simulation and pump data into Target array
            for (int i = lala.Rowstartindex; i < lala.Rowendindex; i++) //initial price
            {
                lala.Target_arr[i , 0] = lala.Opt.S;
            }
            for (int i = lala.Rowstartindex; i < lala.Rowendindex; i++) //price random walk
            {
                for (int j = 0; j < step; j++)
                {
                    lala.Target_arr[i , j+1] = lala.Target_arr[i , j] * Math.Exp((rate - vol * vol / 2) * timestep + vol * Math.Sqrt(timestep) * lala.Input_rand[i, j]);
                }
            }
        }
        ...
        public static double[] deltaCV(Option haha3, double[,] hahaha, bool multi)
        {
            //how many rows do we have
            int row = hahaha.GetLength(0);
            //matrix that stores price change information
        ...
        static void fill_rows_deltaCV(object notreally)//Again, not really all objects fit in
        {
            ...
            EuroOption hehe1 = new EuroOption(lala.Opt.CALL, lala.Opt.K, lala.Opt.S, lala.Opt.T, lala.Opt.R, lala.Opt.Sig);

            //get size of Target double[]
            int col = lala.Target_arr_cv.GetLength(0);
            ...
            for (int i = lala.Rowstartindex; i < lala.Rowendindex; i++) //price random walk
            {
                for (int j = 0; j < step; j++)
                {
                    ...
                    lala.Target_arr_cv [i] = lala.Target_arr_cv [i] + deltaj * (priceevol[i] - priceevol0[i] * onestepinc);
