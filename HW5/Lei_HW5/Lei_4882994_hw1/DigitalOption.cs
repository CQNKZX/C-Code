...
{
    sealed class DigitalOption : Option
    {
        public double Rebate { get; set; }
        public DigitalOption(bool call, double k, double s, double t, double r, double sig, double rebat)
        {
            CALL = call; K = k; S = s; T = t; R = r; Sig = sig; Rebate = rebat;
        }
        //compute price with simulator and backward reduction, anti means antithetic reduction, CV means control variate
        public double priceoption(DigitalOption opt, double[,] randmtx, bool anti, bool CV, bool multi)
        {
        ...
        public double[] bkrdc(DigitalOption opt, double[,] finstat)//backward reduction given final values of option 
        {
            ...
            else if (opt.CALL == false)
            {
                for (int i = 0; i < trial; i++)
                {
                    finstate[i] = ((finstat[i, lastcol] - opt.K) < 0 ? rebatte : 0); ;//put
                }
            }
        ...
        public double[] bkrdc(DigitalOption opt, double[,] finprice, double[] deltaCV)
        {
            ...
            if (opt.CALL == true) //calculate option value at maturity
            {
                for (int i = 0; i < trial; i++)
                {
                    finstate[i] = ((finprice[i, lastcol] - opt.K) > 0 ? rebatte : 0);
                    finstate[i] = finstate[i] - deltaCV[i];//call
                }
            }
            else if (opt.CALL == false)
            {
        ...
        public double[] greeks(DigitalOption opt, double[,] randmtx, bool anti, bool CV, bool multi)//calculate greeks for european option
        {
            ...
            DigitalOption iori0 = new DigitalOption(opt.CALL, opt.K, opt.S * 0.99, opt.T, opt.R, opt.Sig, opt.Rebate);
            DigitalOption iori1 = new DigitalOption(opt.CALL, opt.K, opt.S * 1.01, opt.T, opt.R, opt.Sig, opt.Rebate);
            ...
            DigitalOption haha0 = new DigitalOption(opt.CALL, opt.K, opt.S, opt.T, opt.R, opt.Sig * 0.999, opt.Rebate);
            p0 = opt.priceoption(haha0, randmtx, anti, CV, multi);
            output[4] = (price - p0) / (0.001 * opt.Sig);//vega
            double incretime = opt.T * 0.01;
            DigitalOption haha22 = new DigitalOption(opt.CALL, opt.K, opt.S, opt.T + incretime, opt.R, opt.Sig, opt.Rebate);
            p1 = opt.priceoption(haha22, randmtx, anti, CV, multi);
            output[5] = -(p1 - price) / (incretime);//theta
            DigitalOption haha000 = new DigitalOption(opt.CALL, opt.K, opt.S, opt.T, opt.R * 0.999, opt.Sig, opt.Rebate);
