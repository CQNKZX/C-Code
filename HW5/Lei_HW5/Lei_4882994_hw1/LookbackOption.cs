...
{
    sealed class LookbackOption : Option
    {
        public LookbackOption(bool call, double k, double s, double t, double r, double sig)
        {
        ...
        public double priceoption(LookbackOption opt, double[,] randmtx, bool anti, bool CV, bool multi)
        {
        ...
        public double[] bkrdc(LookbackOption opt, double[,] finstat)//backward reduction given final values of option 
        {
            ...
            if (opt.CALL == true) //calculate option value at maturity
            {
                ...
                {
                    finstate[i] = Math.Max(0, rowmax[i] - opt.K);//call
                }
            }
            else if (opt.CALL == false)
            {
                ...
                for (int i = 0; i < trial; i++)
                {
                    finstate[i] = Math.Max(0, opt.K - rowmin[i]);//put
                }
            }
        ...
        public double[] bkrdc(LookbackOption opt, double[,] finstat, double[] deltaCV)
        {
            int trial = finstat.GetLength(0);
            int lastcol = finstat.GetLength(1) - 1;
            double[] rowmax = new double[1];
            ...
            if (opt.CALL == true) //calculate option value at maturity
            {
                ...
                for (int i = 0; i < trial; i++)
                {
                    finstate[i] = Math.Max(0, rowmax[i] - opt.K) - deltaCV[i];//call
                }
            }
            else if (opt.CALL == false)
            {
                ...
                for (int i = 0; i < trial; i++)
                {
                    finstate[i] = Math.Max(0, opt.K - rowmin[i]) - deltaCV[i];//put
                }
            }
            double[] inistate = new double[trial]; //store backward reduction value of option
            for (int i = 0; i < trial; i++)
        ...
        public double[] greeks(LookbackOption opt, double[,] randmtx, bool anti, bool CV, bool multi)//calculate greeks for european option
        {
            ...
            LookbackOption iori0 = new LookbackOption(opt.CALL, opt.K, opt.S * 0.99, opt.T, opt.R, opt.Sig);
            LookbackOption iori1 = new LookbackOption(opt.CALL, opt.K, opt.S * 1.01, opt.T, opt.R, opt.Sig);
            ...
            LookbackOption haha0 = new LookbackOption(opt.CALL, opt.K, opt.S, opt.T, opt.R, opt.Sig * 0.999);
            p0 = opt.priceoption(haha0, randmtx, anti, CV, multi);
            output[4] = (price - p0) / (0.001 * opt.Sig);//vega
            double incretime = opt.T * 0.01;
            LookbackOption haha22 = new LookbackOption(opt.CALL, opt.K, opt.S, opt.T + incretime, opt.R, opt.Sig);
            p1 = opt.priceoption(haha22, randmtx, anti, CV, multi);
            output[5] = -(p1 - price) / (incretime);//theta
            LookbackOption haha000 = new LookbackOption(opt.CALL, opt.K, opt.S, opt.T, opt.R * 0.999, opt.Sig);
