    ...
    public partial class Option_GUI : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (radioCall.Checked)
            {
                ...
                try
                {
                    textSys.Text = "Computation in process...";//computation load stop this message from displaying
                    ...
                    EuroOption europ = new EuroOption(boo, k, s, t, r, vol);
                    randnum radmx = new randnum();
                    double[,] rndmatx = radmx.rndmtx(tril, stps);
                    double[] prc = europ.priceoption(europ, rndmatx);
                    double[] grks = europ.greeks(europ, rndmatx);
                    textOpt.Text = Convert.ToString(prc[0]);
                    textSE.Text = Convert.ToString(prc[1]);
            ...
            else if (radioPut.Checked)
            {
                ...
                try
                {
                    textSys.Text = "Computation in process...";
                    k = Convert.ToDouble(textK.Text);
                    s = Convert.ToDouble(textS.Text);
                    t = Convert.ToDouble(textT.Text);
                    r = Convert.ToDouble(textR.Text);
