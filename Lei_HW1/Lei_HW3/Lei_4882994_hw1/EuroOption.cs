...
namespace Lei_4882994_hw1
{
    sealed class EuroOption : Option
    {
        ...
        public double priceoption(EuroOption opt, double[,] randmtx, bool anti, bool CV)
        {
            double[] finprice1 = new double[1];
            double[] finprice2 = new double[1];
            ...
            finprice1 = Simulator1.priceevol(opt, randmtx);//final price
            if (anti == true)
            {
                randmtx1 = auxiliary.negativearray(randmtx);
                finprice2 = Simulator1.priceevol(opt, randmtx1);//final price if antithetic reduction used
            }

            //calculate deltaCV if necessary
            if (CV == true)
            {
                deltaCV1 = Simulator1.deltaCV(opt, randmtx);//final price
                if (anti == true)
                {
                    randmtx1 = auxiliary.negativearray(randmtx);
                    deltaCV2 = Simulator1.deltaCV(opt, randmtx1);//final price
                }
            }
        ...
        public double[] bkrdc(EuroOption opt, double[] finprice, double[] deltaCV)
        {
            ...
            if (opt.CALL == true) //calculate option value at maturity
            {
                for (int i = 0; i < trial; i++)
                {
                    finstate[i] = Math.Max(0, finprice[i] - opt.K) - deltaCV[i];//call
                }
            }
            else if (opt.CALL == false)
            {
                for (int i = 0; i < trial; i++)
                {
                    finstate[i] = Math.Max(0, opt.K - finprice[i]) - deltaCV[i];//put
                }
            }
            double[] inistate = new double[trial]; //store backward reduction value of option
            for (int i = 0; i < trial; i++)
        ...
        public double[] greeks(EuroOption opt, double[,] randmtx, bool anti, bool CV)//calculate greeks for european option
        {
            ...
            double p0 = iori0.priceoption(iori0, randmtx, anti, CV);
            double p1 = iori1.priceoption(iori1, randmtx, anti, CV);
            ...
            p0 = opt.priceoption(haha0, randmtx, anti, CV);
