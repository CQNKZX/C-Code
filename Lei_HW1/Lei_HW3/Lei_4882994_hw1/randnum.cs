...
namespace Lei_4882994_hw1
{
    sealed class randnum //whenever you need random numbers, come here
    {
        ...
        {
            if (row % 2 != 0) row = row + 1;//in my progrma you cannot run AVR if trial is odd number, so fix it by adding 1
            int row1 = row / 2;
            double[,] half = new double[row1,col];
            randnum biubiubiu = new randnum();//instantiate
            half = biubiubiu.rndmtx(row1, col);// call method
            return half;
